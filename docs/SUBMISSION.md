...
Either of you can start a conversation — you walk up and click it, or it walks up to you and speaks first. Either way the game hands the agent the situation (who is speaking, where, what time it is in-game, what you chose on the previous turn) and the agent writes **the robot's spoken line and the one-to-four replies you are offered**, including the one that ends the exchange, so even the goodbye is in character. Those lines were never written by the game's author. They render in the game's own dialogue window, in the same panel, in the same font, indistinguishable from hand-authored content.
...
It can join and leave, look around, walk to people and rooms **by name**, step and turn like a human on WASD, speak aloud, open a conversation, and author its own character's dialogue.

It cannot move your camera, choose your dialogue options, give itself money or reputation, set game flags, or teleport past collision. Those tools do not exist — two that did were deleted rather than shipped. Destinations are named rather than coordinates, so the agent cannot place itself inside a desk; it walks with the same pathfinding, the same walk cycle, and the same collision as every other character in the office.
...
- **Authored dialogue** (`src/webmcp/agent-dialogue.ts`). WebMCP has no way for a page to push to an agent — every interaction is agent-initiated. But tool calls are *awaited*, so `wait_for_player_message` simply holds its call open and resolves the instant the player clicks: a notification, built out of a protocol that has none. Measured at 2.6 s against a 25 s ceiling. When it does time out it returns a plain "nothing yet", degrading into ordinary polling, so a slow or disconnected agent can never freeze the human's game.
- **A briefing, not just tools** (`get_instructions`). Tool descriptions are too small to carry a multi-step protocol, so the agent can ask for the whole thing in plain language — how to join, how to move, how the conversation loop works. It reads like a skill.
...
3. Ask your agent: *"Read the game's instructions, then join as a coworker called Rusty who is a sarcastic QA engineer. Walk over to Bartek, say hello, then come back and start a conversation with me."*
4. Answer it. Ask your agent to keep going — it is writing every line you read, and it hears your reply the moment you click.
...
5. **1:20** Let the robot open the conversation itself. **Say plainly: the game's author never wrote these lines.**
6. **1:50** Pick a reply. Show the agent hearing it immediately and answering — the reply reaches it on the click, not on a poll.
