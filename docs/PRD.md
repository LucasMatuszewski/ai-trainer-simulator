...
- **Lunch window + staggering** (in `npc-controller.ts` + `npc-schedule.ts`). During the first 60 s of the afternoon period, `pickRandomDestination` raises the kitchen probability from 10% to 60% for `SOCIAL_LUNCHERS` (default: all except Burek and the CEO). The lunch fire is staggered per NPC by `LUNCH_STAGGER_OFFSET(npcId, day, rng) ∈ [0, 2] s`. `LUNCH_OUTSIDERS` (default: Maciek, Marek) instead have a 30% chance to go to the kitchen outside the lunch window (eat alone) and a 30% chance to skip the lunch entirely.
- **Lunch-only dialogues** (`src/engine/bubbles.ts`). New `LUNCH_DIALOGUES` array of 50+ funny lines (IT / startup / gaming / AI / coffee / food / diet / beer / pizza / vege / eco / work). When both NPCs in a bubble pair are in the `kitchen` state, `pickLine` is called with `dialogueContext: "lunch"`. `LUNCH_DIALOGUES` and `INTER_NPC_LINES` are kept strictly separate — a line in one is never in the other.
...
- **Now:**
  ...
  5. **Lunch window with staggering and "outsiders".** During the morning-to-afternoon transition (i.e. the first 60 s of the afternoon period), the random-walk layer raises the kitchen stay probability from 10% to 60% for NPCs flagged as "social lunchers" (default: all except Burek and the CEO). NPCs flagged as "outsiders" (default: Maciek, Marek — assigned by Lucas) have a 30% chance to go to the kitchen outside the lunch window and a 30% chance to skip the lunch entirely. Lunch walkers are queued with a 1-2 s per-NPC random offset (stagger), so they do not all start at the same instant. The stagger is per-day via the seeded RNG so it is reproducible.
  6. **Lunch-only dialogue pool.** New constant `LUNCH_DIALOGUES` in `src/engine/bubbles.ts` (50+ lines, GLM-authored per the brief: IT jokes, startup jokes, gaming, AI, coffee, dinner, farting, diet, fat, beer, pizza, vege, eco, work). When two NPCs are within 2.5 m of each other AND both have the `kitchen` state (or are in the lunch window), the bubble trigger picks from `LUNCH_DIALOGUES` instead of `INTER_NPC_LINES`. The two pools are kept strictly separate — a line used in `LUNCH_DIALOGUES` is never reused in `INTER_NPC_LINES` and vice versa. The bubble trigger code is the same; the line source switches based on `(stateA, stateB) === ("kitchen", "kitchen")` or on the lunch-window flag.
