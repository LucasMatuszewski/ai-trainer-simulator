...
- **Waypoint graph** (`src/content/corridor-waypoints.ts`). 20-30 hand-authored waypoints: every doorway, every corridor midpoint, every meaningful in-room spot (kitchen fridge / coffee / microwave / sink / table, meeting table center, toilet stall 1 / sink, training row 1/2/3, CEO doorway, every desk's front-of-chair spot). Edges between waypoints are precomputed; any edge that would cross a furniture AABB is removed. The graph is pure data, easy to update when furniture moves (C-44 re-layouts, etc.).
- **A\* planner** (`src/engine/npc-path.ts`, pure function `planNpcPath(from, to, waypoints, edges, obstacles)`). Binary-heap open set, Euclidean heuristic, returns the sequence of `Vector3` waypoints (including the `from` and `to`) the NPC should walk through. If no graph route exists, falls back to a direct two-point path plus depenetration of the endpoint. Returns `null` only when no walk is possible (caller keeps the NPC where it is).
...
- **Lunch-only dialogues** (`src/content/lunch-dialogues.ts`, NEW). New `LUNCH_DIALOGUES_HUMAN: string[]` (~45 lines) and `LUNCH_DIALOGUES_DOG: string[]` (~5-8 lines: "woof!", "*tail wag*", "*sniff*", etc.) of funny lines (IT / startup / gaming / AI / coffee / food / diet / beer / pizza / vege / eco / work). When both NPCs in a bubble pair are in the `kitchen` state, `pickLine` is called with `dialogueContext: "lunch"` and the speaker's id; if the speaker is Burek, `LUNCH_DIALOGUES_DOG` is used; otherwise `LUNCH_DIALOGUES_HUMAN`. All three pools (`LUNCH_DIALOGUES_HUMAN`, `LUNCH_DIALOGUES_DOG`, `INTER_NPC_LINES`) are kept strictly separate — a line in one is never in another. Lines must be ≤ 60 chars (the bubble canvas is hardcoded to 32 chars × 2 lines; staying under 60 avoids the `...` truncation in `bubbles.ts:fitLine`).
- **Local NPC-NPC avoidance** (in `npc-controller.ts`, pure `computeAvoidancePush(self, others, radius)`). Each frame, for each walking NPC, sample the 2 nearest NPCs; if within 1.5 m and both moving, apply a 0.3 m lateral push perpendicular to velocity, away from the other. Priority order: player > Burek > CEO > named NPCs > the rest. This is what makes lunch-time NPCs "walk together and stand together, like they are talking" without stacking.
...
- **Section changed:** `src/content/corridor-waypoints.ts` (NEW), `src/engine/npc-path.ts` (NEW), `src/engine/npc-walk-cycle.ts` (NEW), `src/engine/npc-controller.ts` (replace 2s linear lerp with path-advance), `src/content/npc-schedule.ts` (expand `RANDOM_DESTINATIONS` + add `KITCHEN_MICRO_STOPS` + add `LUNCH_DIALOGUES`), `src/engine/bubbles.ts` (add `LUNCH_DIALOGUES` pool + trigger from the lunch window), §11.1 (NPC schedule), §11.3 (NPC walk animation), new sub-section §11.6 (NPC path-following architecture).
...
- **Now:**
  ...
  6. **Lunch-only dialogue pool.** New constant `LUNCH_DIALOGUES` in `src/content/lunch-dialogues.ts` (50+ lines, **50 specifically — confirmed by Lucas on 2026-08-31**). The pool has two sub-arrays so the bubble trigger can pick the right tone: `LUNCH_DIALOGUES_HUMAN: string[]` (~45 lines) for the people, and `LUNCH_DIALOGUES_DOG: string[]` (~5-8 lines) for Burek's bark / whine / pant sounds rendered as text ("woof!", "bork bork!", "*sniff sniff*", "*tail wag*", "feed me pls"). Topics: IT jokes, startup jokes, gaming, AI, coffee, dinner, farting, diet, fat, beer, pizza, vege, eco, work. Tone: IT Crowd + Silicon Valley, dry, deadpan, brutally accurate to real IT-folk suffering, occasional absurd escalation (per the project comedy DNA in `docs/PRD.md` §4.3 and the GLM comedy brief). Lines must be **≤ 60 chars** (the bubble canvas is hardcoded to 32 chars × 2 lines = 61 max; staying under 60 avoids the `...` truncation in `fitLine` at `src/engine/bubbles.ts:77-88`). When two NPCs are within 2.5 m of each other AND both have the `kitchen` state (or are in the lunch window), the bubble trigger picks from `LUNCH_DIALOGUES` instead of `INTER_NPC_LINES`. If one of the two NPCs is Burek, `LUNCH_DIALOGUES_DOG` is used; otherwise `LUNCH_DIALOGUES_HUMAN`. The two pools (and the work pool) are kept strictly separate — a line used in one is never reused in another. The bubble trigger code is the same; the line source switches on `(stateA, stateB) === ("kitchen", "kitchen")` and on the `isBurek` flag.
...
- **Implementation:**
  ...
  - New `src/engine/npc-path.ts` — `planNpcPath(from, to, waypoints, edges, obstacles) → Vector3[] | null`. Pure function. A* on the waypoint graph (binary-heap open set), falls back to direct path + depenetration if no graph route. The unit test in `tests/unit/npc-path.test.ts` covers: direct path with no obstacles, blocked direct path that must route through a waypoint, no path returns null, depenetration of the endpoint, A* tie-breaking (shorter preferred when costs are equal).
  ...
  - `src/content/npc-schedule.ts` — add `walkSpeed: number` per NPC in `NPCS` (or a per-id map). Add `KITCHEN_MICRO_STOPS`, `KITCHEN_STOP_DWELL`, `pickKitchenSequence(rng, npcId) → ScheduleEntry[]` (Fisher-Yates a 3-4-element subset, prepend the entry, append the desk). Add `SOCIAL_LUNCHERS: ReadonlySet<NpcId>` and `LUNCH_OUTSIDERS: ReadonlySet<NpcId>` (default outsiders: Maciek, Marek per Lucas; configurable). `pickRandomDestination` raises kitchen probability to 60% during the lunch window for `SOCIAL_LUNCHERS` and to 30% outside it for `LUNCH_OUTSIDERS`. A new `LUNCH_STAGGER_OFFSET(npcId, day, rng) → number` returns 0-2 s.
  - `src/engine/bubbles.ts` — add `LUNCH_DIALOGUES` (50+ lines, GLM-authored). The `pickLine` function takes a `dialogueContext: "work" | "lunch"` parameter; the controller passes `"lunch"` when both NPCs in a bubble pair are in the `kitchen` state.
  ...
  - **Acceptance criteria:** (1) Period transitions no longer teleport NPCs through walls (visible in a Playwright screenshot of an afternoon kitchen moment). (2) When morning.position === afternoon.position, the NPC does not bob/sway in place. (3) An NPC in the kitchen state visits 3-4 of {fridge, coffee, sink, microwave, table} in a random order, dwells 4-10 s at each, then returns to the desk. (4) During the lunch window, 3-5 NPCs are in the kitchen at the same time, with 1-2 s stagger offsets; they don't stack. (5) `LUNCH_DIALOGUES` lines are never used outside the kitchen or lunch window, and `INTER_NPC_LINES` lines are never used in the kitchen. (6) TDD per PR-8: all four new pure functions have failing tests first, then implementations.
